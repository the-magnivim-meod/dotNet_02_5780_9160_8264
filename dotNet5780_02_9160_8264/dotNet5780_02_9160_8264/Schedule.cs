...
namespace dotNet5780_02_9160_8264
{
    ...
    class Schedule
    {
        ...
        {
            int num_of_nights = ((int)guestReq.ReleaseDate.MonthIndex - (int)guestReq.EntryDate.MonthIndex) * 31 + (ending_day.DayIndex - guestReq.EntryDate.DayIndex);// calculates the length of visit in nights
            bool allow = true;//is the booking still relevant
            for (int i = 0; i < num_of_nights && allow; i++)
            {
